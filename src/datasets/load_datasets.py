...
from ..utils.dataset_L import Language, caption_collate_fn
...
def get_FL_trainloader(dataset, data_root, num_clients, partition, alpha, batch_size):
    ...
    class_label = torch.Tensor(np.array(range(class_num)))
    ...
    net_dataidx_map = data_partitioner(dataset, num_samples,num_clients, partition=partition,
                                       check_dir="/root/newMFL/data_partition/", alpha=alpha,
                                       y_train=np.array(train_set.targets))
    # if num_clients < 10:
    #     net_dataidx_map = dict(islice(net_dataidx_map.items(), num_clients))
    ...
    return loader_map, test_loader, class_label


def data_partitioner(dataset, num_samples, num_nets, partition='homo', check_dir=None, alpha=0.5, y_train=None):
    '''
    num_nets:  the number of client
    '''
    check_dir = check_dir + f'{num_nets}_client_{dataset}'
    ...
    if partition == "homo":
        ...
        if os.path.isfile(check_dir):
            ...
            while min_size < (10 if dataset == "cifar100" else (3000 if dataset == "AG_NEWS" else 500)):
                ...
                for k in range(K):
                    idx_k = np.where(y_train == k)[0] # the index whose class is k
