...
class ClientTrainer:
    ...
    def run(self, agg_proto, agg_model,  clip_enc):
        ...
        self.shared_model.to(self.gpuid)
        
        if self.cur_epoch>0:
            if agg_model['image']:
                # print(agg_model['image'])
                agg_model_image_state_dict = {key: value for key, value in agg_model['image'].items()}
                # print(agg_model_image_state_dict)
                self.shared_model.load_state_dict(agg_model_image_state_dict, strict=False)
            else:
                agg_model_text_state_dict = {key: value for key, value in agg_model['text'].items()}
                self.shared_model.load_state_dict(agg_model_text_state_dict, strict=False)
        # else:
        #     if agg_model['image']:
        ...
        for i in range(self.local_epochs):
            if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
                model_path = f'./results/sm/img/Client{self.client_idx}-global_{self.cur_epoch}-model_Local{self.local_epoch}.pth'
            else:
                model_path = f'./results/sm/text/Client{self.client_idx}-global_{self.cur_epoch}-model_Local{self.local_epoch}.pth'
        ...
        if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
            ...
            with torch.no_grad():
                for i,data in enumerate(self.traindata):
                    inputs, target =data
                    inputs = inputs.to(self.gpuid)
                    target = target.to(self.gpuid)
                    output,_  = self.model(clip_enc.img_enc(inputs))  
            ...
            with torch.no_grad():
                for i,data in enumerate(self.traindata):
                    ...
                    # inputs = inputs.to(self.gpuid)
                    target = target.to(self.gpuid)
                    output,_ = self.model(clip_enc.txt_enc(inputs,False))  
                    embeddings.append(output)
                    labels.append(target)
        ...
        for class_id in range(self.config.project_head.class_num):
            ...
            print(f'{class_id} number: {class_embeddings.size(0)}')
            if class_embeddings.size(0) > 0:
                if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
                   
                    prototypes['image'].append(class_embeddings.mean(dim=0))
                    proto_fig['image'].append(class_embeddings.mean(dim=0))
                elif  self.dset_name == 'AG_NEWS' or self.dset_name == 'YelpReviewPolarity':
                    prototypes['text'].append(class_embeddings.mean(dim=0))
                    proto_fig['text'].append(class_embeddings.mean(dim=0))
            else:
                if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
                    
                    proto_fig['image'].append(None)
                elif  self.dset_name == 'AG_NEWS' or self.dset_name == 'YelpReviewPolarity':
                    proto_fig['text'].append(None)

        pca = PCA(n_components=2)
        embeddings_2d = pca.fit_transform(embeddings.cpu().numpy())
        plt.figure(figsize=(12, 8))
        unique_classes = np.unique(labels.cpu().numpy())
        # Plot embeddings with different colors per class
        for class_id in unique_classes:
            class_points = embeddings_2d[labels.cpu() == class_id]
            
            plt.scatter(class_points[:, 0], class_points[:, 1],
                    marker='o', label=f'Class {class_id}', alpha=0.6)

        # Check if the dataset is 'cifar100' or 'cifar10'
        if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
            # Create a dictionary to store category labels for the legend
            category_labels = {}

            # Filter out None values from the prototypes and keep the valid indices
            valid_prototypes = [(i, proto) for i, proto in enumerate(proto_fig['image']) if proto is not None]
    
            # If valid prototypes exist, stack and apply PCA
            if valid_prototypes:
                valid_prototypes_tensor = torch.stack([proto for _, proto in valid_prototypes])
                prototypes_2d = pca.fit_transform(valid_prototypes_tensor.cpu().numpy())
        
                # Plot the prototypes, ensuring the category is kept in the legend
                for idx, (x, y) in enumerate(prototypes_2d):
                    category_id = valid_prototypes[idx][0]  # Get the original index (category ID)

                    # Store the category label for the legend
                    if category_id not in category_labels:
                        category_labels[category_id] = f'Class {category_id}'
                
                    plt.scatter(x, y, label=category_labels[category_id], s=100, marker='s', edgecolor='black', linewidth=1.5)

        # Check if the dataset is 'AG_NEWS' or 'YelpReviewPolarity'
        elif self.dset_name == 'AG_NEWS' or self.dset_name == 'YelpReviewPolarity':
            # Create a dictionary to store category labels for the legend
            category_labels = {}

            # Filter out None values from the prototypes and keep the valid indices
            valid_prototypes = [(i, proto) for i, proto in enumerate(proto_fig['text']) if proto is not None]
    ...
    def setModel(self,config):
        ...
        if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
            ...
            self.shared_model = ImageNet(config.project_head.input_embedding, config.project_head.out_embedding, config.project_head.class_num, config.project_head.norm, config.project_head.hid_num, config.project_head.c_hid)
            ...
            self.criterion = nn.CrossEntropyLoss()
            self.shared_model = TextNet(config.project_head.input_embedding, config.project_head.out_embedding, config.project_head.class_num, config.project_head.norm, config.project_head.hid_num, config.project_head.c_hid)
    ...
    # step 3: Learning
    ##################################################
    def tra(self, agg_proto, clip_enc):
        ...
        for i, data in enumerate(self.traindata):
            ...
            with torch.set_grad_enabled(True):
                if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
                    ...
                    # print(labels_var.min(), labels_var.max())
                    emb =  clip_enc.img_enc(inputs_var)
                    ...
                    if agg_proto:
                        ...
                        self.shared_model.freeze_grad()
                        out_s, out_norm_s = self.shared_model(emb)
                        image_logits_s = self.shared_model.classify(out_norm_s)
                        l_d = disll(out_norm,out_norm_s)
                        loss_s = self.criterion(image_logits_s, labels_var) 
                        loss_p = PrototypeLoss(agg_proto)
                        l_p = loss_p(out_norm, out_norm, re=False)
                        l_o = loss_o.detach().clone()
                        loss = loss_o + l_p+ (l_o/loss_s)*l_d
                        # print(f"loss:{loss}")
                        # print(f"proto loss:{l_p}")
                    else:
                        loss = loss_o
                        
                    
                    _, preds = torch.topk(image_logits, k=5, dim=1)  
                    correct = preds.eq(labels_var.view(-1, 1).expand_as(preds))  
                    ...
                    labels_bt = labels_bt.to(self.gpuid)
                    emb = clip_enc.txt_enc(inputs_bt, False)
                    out, out_norm = self.model(emb)
                    ...
                    if agg_proto:
                        ...
                        self.shared_model.freeze_grad()
                        out_s, out_norm_s = self.shared_model(emb)
                        text_logits_s = self.shared_model.classify(out_norm_s)
                        l_d = disll(out_norm,out_norm_s)
                        loss_s = self.criterion(text_logits_s, labels_bt) 
                        loss_p = PrototypeLoss(agg_proto)
                        l_p = loss_p(out_norm, out_norm, re=False)
                        l_o = loss_o.detach().clone()
                        loss = loss_o + l_p+ (l_o/loss_s)*l_d
                    ...
                    _, preds = torch.topk(text_logits, k=2, dim=1)  
                    correct = preds.eq(labels_bt.view(-1, 1).expand_as(preds))  
        ...
        with torch.no_grad():
            ...
            for data in self.testdata:
                if self.dset_name == 'cifar100' or self.dset_name == 'cifar10':
                    ...
                    out, out_norm= self.model(clip_enc.img_enc(inputs_var))
                    image_logits = self.model.classify(out)
                    ...
                    inputs_bt, labels_bt = data
                    labels_bt = labels_bt.to(self.gpuid)
                    out, out_norm = self.model(clip_enc.txt_enc(inputs_bt,False))
                    text_logits = self.model.classify(out)
